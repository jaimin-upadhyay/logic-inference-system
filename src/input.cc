...
// then stores all the expected number of queries from the following lines in the input stream
void Input::ReadQueries(std::ifstream &input_stream, std::vector<std::string> *queries) {
    int no_of_queries = 0;
    input_stream >> no_of_queries;
    // Ignore remaining characters in the line after the number of queries to avoid confusion in following stream reads
...
// then stores all the expected number of sentences from the following lines in the input stream to the knowledge base
void Input::ReadKnowledgeBase(std::ifstream &input_stream, std::vector<std::string> *knowledge_base) {
    int no_of_sentences = 0;
    input_stream >> no_of_sentences;
    // Ignore remaining characters in the line after the value of number of sentences to avoid confusion in following stream reads
    input_stream.ignore(std::numeric_limits<std::streamsize>::max(), '\n');
    std::string temp_string;
...
// then reads the queries and knowledge base from the input file stream based on the specified format
